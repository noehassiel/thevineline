                    ...
                    <div class="shop-details-flex-wrap d-flex">
                                ...
                                @foreach ($product->images as $image)
                                    <li class="nav-item list-group-item" role="presentation">
                                        <a class="nav-link" id="profile-tab" data-bs-toggle="tab"
                                            data-bs-target="#profile_{{ $image->id }}" type="button" role="tab"
                                            aria-controls="profile" aria-selected="false">
                                            <img width="80"
                                                height="80"src="{{ asset('img/products/' . $image->image) }}"
                                                alt="" class="img-fluid">
                                        </a>
                                    </li>
                                @endforeach
                        ...
                        <div class="shop-details-img-wrap ">
                            <div class="tab-content" id="myTabContent">
                                ...
                                @foreach ($product->images as $image)
                                    <div class="tab-pane fade" id="profile_{{ $image->id }}" role="tabpanel"
                                        aria-labelledby="profile-tab">
                                        <img width="400"
                                            height="400"src="{{ asset('img/products/' . $image->image) }}"
                                            alt="" class="img-fluid">
                                    </div>
                                @endforeach
                            </div>
                ...
                <div class="col-md-6">
                    <div class="shop-details-content">
                        ...
                        @if ($product->type == 'subscription')
                            ...
                            <ul class="list-unstyled">
                                @foreach ($product->characteristics as $characteristic)
                                    <li><ion-icon name="checkmark-circle" class="text-success"></ion-icon>
                        ...
                        @if ($product->has_variants == true)
                            <div class="product-details-info mt-4">
                                ...
                                @if ($product->variants->count() != 0)
                                    <h6 class="wk-variant-title">Escoge tu variante <span class="text-warning">Existencias
                        ...
                        <div class="product-actions d-flex align-items-center mt-5">
                            @switch($product->type)
                                @case('subscription')
                                    <a href="{{ route('checkout.subscription', $product->id) }}"
                                        class="btn btn-primary d-flex align-items-center me-3" role="button">
                                        <ion-icon name="planet-outline" class="me-2"></ion-icon> Comprar esta suscripción
                                    </a>
                                @break

                                @case('digital')
                                    <a href="{{ route('add-cart', ['id' => $product->id, 'variant' => 'digital_product']) }}"
                                        id="addToCartBtn" class="btn btn-primary d-flex align-items-center me-3" role="button">
                                ...
                                @default
                                    @if ($product->has_variants == true)
                                        @if ($product->variants->count() == 0)
                                            <div class="me-3">
                                                <p class="no-existance-btn mb-0"><i class="fas fa-heartbeat"></i> Sin Existencias
                                                </p>
                                                <p class="no-existance-explain mb-0 mt-0"><small>Resurtiremos pronto, revisa más
                                                        adelante.</small></p>
                                            </div>
                                        @else
                                    ...
                                    @else
                                        ...
                                        @else
                                            <a href="{{ route('add-cart', ['id' => $product->id, 'variant' => 'unique']) }}"
                                                id="addToCartBtn" class="btn btn-primary d-flex align-items-center me-3"
                                                role="button">
                                                <ion-icon name="bag-add-outline" class="me-2"></ion-icon> Agregar al carrito
                                            </a>
                                        @endif
                            ...
                            @if (isset(Auth::user()->id) && Auth::user()->isInWishlist($product->id))
                                <a href="{{ route('wishlist.remove', $product->id) }}"
                                    class="btn btn-outline-secondary d-flex align-items-center"><ion-icon
                                        name="heart-dislike-outline" class="me-2"></ion-icon> Quitar de tu Wishlist</a>
                            @else
                                @guest
                                    <a href="{{ route('login') }}"
                                        class="btn btn-outline-secondary d-flex align-items-center"><ion-icon
                                            name="heart-outline"></ion-icon class="me-2"> Agregar a tu Wishlist</a>
                                @else
                                    <a href="{{ route('wishlist.add', $product->id) }}"
                                        class="btn btn-outline-secondary d-flex align-items-center"><ion-icon
                                            name="heart-outline" class="me-2"></ion-icon> Agregar a tu Wishlist</a>
                                @endif
                                @endif
                            </div>
                            ...
                            @if ($kueski_widget == true)
                                <kueskipay-widget data-kpay-color-scheme="black"
                                    data-kpay-widget-font-size="12"></kueskipay-widget>

                                <script id="kpay-advertising-script"
                                    src="https://cdn.kueskipay.com/widgets.js?authorization=0c821e76-e634-47a3-8ecb-cb1bec22da80&integration=API&sandbox=true">
                                </script>
                            ...
                            @if ($shipment_option != null)
                                <p class="text-primary mt-4"><ion-icon name="bag-handle-outline"></ion-icon> Récibelo de
                                    {{ $shipment_option->delivery_time }} al seleccionar <br> {{ $shipment_option->name }} en
                                    tu Checkout.</p>
                            @endif
                            ...
                            @foreach ($product->links as $link)
                                <div class="d-block mb-3">
                                    <a href="{{ $link->url }}" class="btn btn-primary">
                                        @if ($link->icon != null)
                                            <img src="{{ asset('/img/icons/' . $link->icon) }}" alt="icon"
                                                width="120">
                                        @endif
                                        {{ $link->name }}
                                    </a>
                        ...
                        <div class="product-desc-wrap">
                            <div class="tab-content">
                                <ul class="nav nav-tabs" id="myTab" role="tablist">
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link active" id="home-tab" data-bs-toggle="tab"
                                            data-bs-target="#home" type="button" role="tab" aria-controls="home"
                                            aria-selected="true">Información adicional</button>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link" id="materials-tab" data-bs-toggle="tab"
                                            data-bs-target="#materials" type="button" role="tab"
                                            aria-controls="materials" aria-selected="false">Materiales</button>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link" id="reviews-tab" data-bs-toggle="tab"
                                            data-bs-target="#reviews" type="button" role="tab" aria-controls="reviews"
                                            aria-selected="false">Reseñas
                                            ({{ $product->approved_reviews->count() ?? '0' }})</button>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link" id="sizes-tab" data-bs-toggle="tab" data-bs-target="#sizes"
                                            type="button" role="tab" aria-controls="sizes" aria-selected="false">Guia
                                            de tallas</button>
                                    </li>
                                </ul>
                                ...
                                <div class="tab-content mt-4" id="myTabContent">
                                    ...
                                    <div class="tab-pane fade show " id="sizes" role="tabpanel"
                                        ...
                                        <table class="table table-hover table-responsive mt-2 mb-4">
                                            <tbody>
                                                @foreach ($size_charts as $size_chart)
                                                    <tr>
                                                        <th scope="row" style="width: 150px;">{{ $size_chart->name }}
                                                        </th>
