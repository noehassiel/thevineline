                                        ...
                                        <div class="flex-grow-1 ms-3">
                                            ...
                                            @if ($cart_product['item']['has_discount'] == true && $cart_product['item']['discount_end'] > Carbon\Carbon::today())
                                                <div class="wk-price" style="font-size:.8em;">
                                                    ${{ number_format($cart_product['item']['discount_price'], 2) }}</div>
                                                <div class="wk-price wk-price-discounted"
                                                    style="font-size:.7em !important; ">
                                                    ${{ number_format($cart_product['item']['price'], 2) }}</div>
                                            @else
                                                <div class="wk-price" style="font-size:.8em;">
                                                    ${{ number_format($cart_product['item']['price'], 2) }}</div>
                                            @endif
                                            ...
                                            <div class="btn-group align-items-center">
                                                ...
                                                @else
                                                    <a href="{{ route('cart.substract', ['id' => $cart_product['item']['id'], 'variant' => $cart_product['variant']]) }}"
                                                        class="btn btn-qty">-</a>
                                                @endif
                                                <p class="h-100 mb-0 px-2">{{ $cart_product['qty'] }}</p>
                                                <a href="{{ route('cart.add-more', ['id' => $cart_product['item']['id'], 'variant' => $cart_product['variant'], 'qty' => $cart_product['qty']]) }}"
                                                    class="btn btn-qty">+</a>
                                            </div>
                                        </div>

                                        <div class="wk-total-list text-end">
                                            <a href="{{ route('cart.delete', ['id' => $cart_product['item']['id'], 'variant' => $variant]) }}"
                    ...
                    <div class="col-12">
                        <div class="row justify-content-between wk-cart-info">
                            <div class="col-12 col-md-4 order-5 order-md-1">
                                @php
                                    $card_payment = Nowyouwerkn\WeCommerce\Models\PaymentMethod::where(
                                        'supplier',
                                        '!=',
                                        'Paypal',
                                    )
                                    ...
                                    $cash_payment = Nowyouwerkn\WeCommerce\Models\PaymentMethod::where('type', 'cash')
                                        ->where('is_active', true)
                                        ->first();
                                    $paypal_payment = Nowyouwerkn\WeCommerce\Models\PaymentMethod::where(
                                        'supplier',
                                        'Paypal',
                                    )
                                        ->where('is_active', true)
                                        ->first();
                                    $mercado_payment = Nowyouwerkn\WeCommerce\Models\PaymentMethod::where(
                                        'supplier',
                                        'MercadoPago',
                                    )
                                        ->where('is_active', true)
                                        ->first();
                                    $membership = Nowyouwerkn\WeCommerce\Models\MembershipConfig::where(
                                        'is_active',
                                        true,
                                    )->first();
                                @endphp
                                ...
                                <div class="card px-4 pt-3 pb-3 mb-4">
                                    <div class="row">
                                        @if (!empty($card_payment))
                                            <div class="col-6">
                                                <img src="{{ asset('img/icons/card-info.png') }}"
                                                    style="padding-top: 10px; margin-bottom: 5px; height: 35px; width: auto !important;">
                            ...
                            <div class="col-12 col-md-4 order-md-5">
                                <div class="card px-4 pt-3 pb-3 mb-3">
                                    ...
                                    @if ($shipment_option != null)
                                        <p class="text-primary mt-1 mb-4"><ion-icon name="bag-handle-outline"></ion-icon>
                                            Récibelo en {{ $shipment_option->delivery_time }} al seleccionar
                                            {{ $shipment_option->name }} en tu Checkout.</p>
                                    @endif
                                    <div class="list_custom">
                                        <ul class="list-unstyled">
                                            ...
                                            @php
                                                $rule = Nowyouwerkn\WeCommerce\Models\ShipmentMethodRule::where(
                                                    'is_active',
                                                    true,
                                                )->first();
                                            @endphp
                                            ...
                                            @if (!empty($rule))
                                                <div class="alert alert-info d-block">
                                                    <span
                                                        style="margin-right: 2px"><strong>{{ $rule->type }}</strong></span>
                                                    <span style="margin-right: 2px">cuando
                                                        <strong>{{ $rule->condition }}</strong> sea</span>
                                                    ...
                                                    <span
                                                        style="margin-right: 2px"><strong>{{ number_format($rule->value) }}</strong></span>
                                                </div>
                                            @endif
                                            ...
                                            <li class="d-flex justify-content-between mb-3">
                                                <span class="subtitle">Total</span> <span
                                                    class="amount">${{ number_format($total, 2) }}</span>
                                            </li>

                                            @if (!empty($membership))
                                                <li class="d-flex justify-content-between">
                                                    <span>Puntos para acumular</span><span
                                                        class="points">{{ $points }}</span>
                                                </li>
                                            @endif
                                        </ul>
                                        ...
                                        <div class="row">
                                            @if (!empty($card_payment))
                                                <div class="col-12">
                                                    <a class="btn btn-primary mb-2 w-100 btn_icon" style="padding: 15px;"
                                                        href="{{ route('checkout') }}">
                                            ...
                                            @if (!empty($paypal_payment))
                                                <div class="col pe-1">
                                                    <a class="btn btn-light mb-2 w-100 btn_icon"
                                                        href="{{ route('checkout') }}">
                                                        <img src="{{ asset('assets/img/brands/paypal.png') }}"
                                                            style="padding-top: 10px; margin-bottom: 5px; height: 35px; width: auto !important;">
                                                    </a>
                                ...
                                <a href="{{ route('catalog.all') }}" class="btn btn-link text-center d-block mb-5">Seguir
                                    comprando</a>
                            </div>
                        </div>
                    </div>
